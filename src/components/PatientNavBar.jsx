...
const PatientNavBar = ({ username }) => {
  ...
  const fetchUser = useCallback(async () => {
    try {
      setLoading(true);
      const response = await fetch(`http://railway-backend-production-a8c8.up.railway.app/patientdetails/${username}`);
  ...
  const handleLogout = async () => {
    try {
        const response = await fetch(`http://railway-backend-production-a8c8.up.railway.app/patientlogout/${username}`, {
        method: 'POST',
        });
