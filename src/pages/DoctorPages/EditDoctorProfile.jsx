...
const EditDoctorProfile = () => {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        const response = await fetch(`http://railway-backend-production-a8c8.up.railway.app/doctordetails/${loggedInUsername}`);
        if (response.ok) {
  ...
  const handleEdit = async () => {
    ...
    try {
      const url = new URL(`http://railway-backend-production-a8c8.up.railway.app/editdoctor/${loggedInUsername}`);
      const userData = {
        user: {
          username,
          firstName,
  ...
  const handleNewPassword = async () => {
    ...
    try {
      const response = await fetch('http://railway-backend-production-a8c8.up.railway.app/changePass/doctor', {
        method: 'POST',
        headers: {
  ...
  const handleDeleteUser = async () => {
    try {
      const response = await fetch(`http://railway-backend-production-a8c8.up.railway.app/doctors?userId=${userId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
